...
##############################################
# 3) Block China IP Ranges (CIDR)
# Blocks China-specific IP ranges
##############################################
...
block_china_ips() {
    ...
    echo -e "${GREY}"
    echo -e "${DARK_YELLOW}  • Manage China IP Blocking${NC}"
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    echo -e "${GREY}"
    echo -e "${BLUE}| 1)  - Blocking IP Ranges for China"
    echo -e "${BLUE}| 2)  - Unblocking IP Ranges for China"
    ...
    case $choice in
        1) enable_china_ip_blocking ;;
        2) disable_china_ip_blocking ;;
        3) view_china_ip_rules ;;
...
enable_china_ip_blocking() {
    clear
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    echo -e "${GREY}"
    echo -e "${DARK_YELLOW}  • Blocking IP Ranges for China.${NC}"
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    echo -e "${GREY}"
    ...
    local ipv4_set="CN_ipv4_blocklist"
    local ipv6_set="CN_ipv6_blocklist"
    local iptables_chain="CHINA_BLOCK"
    ...
    if ipset list | grep -q "$ipv4_set"; then
        ...
        for url in https://www.iwik.org/ipcountry/CN.cidr https://www.iwik.org/ipcountry/HK.cidr https://www.iwik.org/ipcountry/TW.cidr; do
            curl -s "$url" | grep -v '^#' | while read -r ip; do
    ...
    if ipset list | grep -q "$ipv6_set"; then
        ...
        for url in https://www.iwik.org/ipcountry/CN.ipv6 https://www.iwik.org/ipcountry/HK.ipv6 https://www.iwik.org/ipcountry/TW.ipv6; do
            curl -s "$url" | grep -v '^#' | while read -r ip; do
    ...
    echo -e "${GREY}"
    echo -e "${GREEN}  ✓ China IPs Blocked Successfully.${NC}"
    sleep 3
    block_china_ips
}

disable_china_ip_blocking() {
    clear
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    echo -e "${GREY}"
    echo -e "${DARK_YELLOW}  • Unblocking IP Ranges for China.${NC}"
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    echo -e "${GREY}"
    ...
    echo -e "${GREEN}  ✓ China IPs Unblocked Successfully.${NC}"
...
source /usr/local/bin/Phoenix_Shield/scripts/rules_persist/rules_persist.sh
    ...
    if ipset list | grep -q "$ipv4_set" || ipset list | grep -q "$ipv6_set"; then
        ...
        echo -e "${GREY}"
        echo -e "${RED}  ✖ No China IP Blocking Rules Found.${NC}"
    fi
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
