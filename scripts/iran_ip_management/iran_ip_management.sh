...
# Description: Manage IRAN IP Ranges(CIDR)
# Author: Phoenix-999
...
source /usr/local/bin/Phoenix_Shield/scripts/essential_packages/essential_packages.sh
...
##############################################
# 2) Block Iran IP Ranges (CIDR)
# Blocks Iran-specific IP ranges
##############################################
...
block_iran_ips() {
    ...
    echo -e "${GREY}"
    echo -e "${DARK_YELLOW}  • Manage Iran IP Blocking${NC}"
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    echo -e "${GREY}"
    echo -e "${BLUE}| 1)  - Blocking IP Ranges for Iran"
    echo -e "${BLUE}| 2)  - Unblocking IP Ranges for Iran"
    ...
    case $choice in
        1) enable_iran_ip_blocking ;;
        2) disable_iran_ip_blocking ;;
        3) view_iran_ip_rules ;;
...
enable_iran_ip_blocking() {
    clear
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    echo -e "${GREY}"
    echo -e "${BLUE}  • Blocking IP Ranges for Iran.${NC}"
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    echo -e "${GREY}"
    ...
    local ipv4_set="IR_ipv4_blocklist"
    local ipv6_set="IR_ipv6_blocklist"
    local iptables_chain="IRAN_BLOCK"
    ...
    if ipset list | grep -q "$ipv4_set"; then
        ...
        curl -s https://www.iwik.org/ipcountry/IR.cidr | grep -v '^#' | while read -r ip; do
    ...
    if ipset list | grep -q "$ipv6_set"; then
        ...
        curl -s https://www.iwik.org/ipcountry/IR.ipv6 | grep -v '^#' | while read -r ip; do
    ...
    echo -e "${GREY}"
    echo -e "${GREEN}  ✓ Iran IPs Blocked Successfully and Persistence Enabled.${NC}"

    sleep 3
    block_iran_ips
}

disable_iran_ip_blocking() {
    clear
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    echo -e "${GREY}"
    echo -e "${BLUE}  • Unblocking IP Ranges for Iran.${NC}"
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    echo -e "${GREY}"
    ...
    echo -e "${GREEN}  ✓ Iran IPs Unblocked Successfully and Persistence Enabled.${NC}"
    sleep 3
    block_iran_ips
}
...
view_iran_ip_rules() {
    ...
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    echo -e "${GREY}"
    echo -e "${BLUE}  • View Blocked IP Ranges.${NC}"
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
    ...
    if ipset list | grep -q "$ipv4_set" || ipset list | grep -q "$ipv6_set"; then
        ...
        echo -e "${GREY}"
        echo -e "${RED}  ✖ No Iran IP Blocking Rules Found.${NC}"
    fi
    echo -e "${DARK_SILVER}${BOLD} ____________________________________________${NC}"
